...
if not EventsFolder then warn("❌ Missing Events Folder") return end

local TaskCompletedEvent = EventsFolder:WaitForChild("TaskCompleted")
local SabotageSuccessEvent = EventsFolder:WaitForChild("SabotageSuccess")
local TaskFailedEvent = EventsFolder:WaitForChild("TaskFailed")
...
print("✅ Minigame Controller [MASTER v13 - FULL] Loaded.")
...
local function SetFrozen(frozen)
	...
	if not frozen then camera.CameraType = Enum.CameraType.Custom end
end
...
local function FailGame()
	...
	-- 1. Tell Server (It decides if we die based on trap status)
	TaskFailedEvent:FireServer(currentTaskNode)
...
local function WinGame()
	...
	if player:GetAttribute("Role") == "Saboteur" then
		print("😈 SABOTAGE ARMED.")
		SabotageSuccessEvent:FireServer(currentTaskNode)
	else
		print("🏆 TASK COMPLETE.")
...
ProximityPromptService.PromptTriggered:Connect(function(prompt, playerWhoTriggered)
	...
	if prompt.Name == "InteractionPrompt" or prompt.Name == "SabotagePrompt" then
		local taskNode = prompt.Parent
		if taskNode then
			print("⚡ Interacted with: " .. taskNode.Name)
			currentTaskNode = taskNode -- ✅ Ensure this is set before starting!
			local name = taskNode.Name

			SafeRun(function()
				if string.find(name, "Badge") or string.find(name, "Scan") then StartBadgeScan(taskNode)
				elseif string.find(name, "Sew") or string.find(name, "Stitch") then StartSewingGame(taskNode)
				elseif string.find(name, "Mannequin") or string.find(name, "Dress") then StartMannequinGame(taskNode)
				elseif string.find(name, "Shoe") or string.find(name, "Shine") then StartShoeShineGame(taskNode)
				elseif string.find(name, "Dye") or string.find(name, "Color") then StartDyeVatGame(taskNode)
				elseif string.find(name, "Steam") or string.find(name, "Valve") then StartSteamGame(taskNode)
				elseif string.find(name, "Chemical") or string.find(name, "Mixer") then StartChemicalGame(taskNode)
				...
				elseif string.find(name, "Safe") or string.find(name, "Security") then StartSecuritySafeGame(taskNode)
				elseif string.find(name, "Measure") or string.find(name, "Cut") then StartMeasureGame(taskNode)
